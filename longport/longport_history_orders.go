...
func longport_history_orders(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "longport_history_orders",
		Description: "History Orders.",
		List: &plugin.ListConfig{
			Hydrate:    list_longport_history_orders,
		...
		Columns: []*plugin.Column{
			...
			{Name: "market", Type: proto.ColumnType_STRING, Transform: transform.FromField("Market"), Description: "Stock Market"},
...
func list_longport_history_orders(ctx context.Context, d *plugin.QueryData, p *plugin.HydrateData) (interface{}, error) {
	...
	var params = trade.GetHistoryOrders{}
	if symbol != "" {
	...
	plugin.Logger(ctx).Info("list_longport_history_orders", "params", params)

	var items []*trade.Order
	items, _, err = context.HistoryOrders(ctx, &params)
	if err != nil {
		return nil, err
	}
